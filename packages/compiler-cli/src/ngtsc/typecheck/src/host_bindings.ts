...
/**
 * Infers the attribute name, binding type, and unit of a bound attribute based on its raw name.
...
function inferBoundAttribute(name: string): {
  ...
  if (name.startsWith(attrPrefix)) {
    ...
    // Style bindings can have a unit suffix: style.width.px
    const styleParts = name.slice(stylePrefix.length).split('.');
    attrName = styleParts[0];
    unit = styleParts.length > 1 ? styleParts[1] : null;
