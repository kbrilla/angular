...
export function getCSSValueCompletions(propertyName: string): ts.CompletionEntry[] {
  const values = getCSSPropertyValues(propertyName);
 ...
 */
const COMMON_LENGTH_UNITS = ['px', 'em', 'rem', '%', 'vh', 'vw'] as const;

/**
 * Time units for animation/transition properties.
 */
const TIME_UNITS = ['s', 'ms'] as const;

/**
 * Angle units for rotation/gradient properties.
 */
const ANGLE_UNITS = ['deg', 'rad', 'turn'] as const;

/**
 * Properties that typically accept length values.
 */
...
/**
 * Properties that typically accept time values.
 */
...
/**
 * Properties that typically accept angle values.
 */
const ANGLE_PROPERTIES = new Set(['rotate', 'transform']);
...
export function getNumericUnitCompletions(
  ...
  // Determine which units are appropriate for this property
  let units: readonly string[];
  if (TIME_PROPERTIES.has(propertyName)) {
    units = TIME_UNITS;
  } else if (ANGLE_PROPERTIES.has(propertyName)) {
    units = ANGLE_UNITS;
  } else {
    // Default to length units for most properties
    units = COMMON_LENGTH_UNITS;
  }
  ...
  for (const unit of units) {
    ...
    completions.push({
      ...
      kindModifiers: '',
      sortText: `0${units
        .indexOf(unit as (typeof COMMON_LENGTH_UNITS)[number])
        .toString()
        .padStart(2, '0')}`,
